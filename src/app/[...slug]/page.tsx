...
const fallbackBody='Este módulo já possui uma rota funcional dentro da estrutura do ZYVO APP. A experiência específica pode ser desenvolvida sobre esta base sem quebrar a Home.';

export default async function FeaturePage({ params }:{ params:Promise<{slug:string[]}> }){
  const {slug}=await params;
  const key=slug.join('/');
  const fallbackTitle=labels[key] ?? slug.map(s=>s.replaceAll('-',' ')).join(' / ');
  const cms=await fetchPublicCmsBundle();
  const page=cms.pages[key];
  const settings=(page?.settings||{}) as Record<string,unknown>;
  const title=page?.title || fallbackTitle;
  const body=typeof settings.body==='string'?settings.body:fallbackBody;
  const backLabel=typeof settings.backLabel==='string'?settings.backLabel:'Voltar ao início';
  return <main style={{minHeight:'100dvh',background:'radial-gradient(circle at 60% 20%,#15293c 0,#07111d 38%,#02070d 75%)',color:'#fff',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif'}}>
    ...
    <div style={{minHeight:'100dvh',display:'grid',placeItems:'center',padding:'112px 32px 32px 104px'}}>
      <section style={{width:'min(720px,92vw)',padding:'42px 44px',border:'1px solid rgba(255,255,255,.1)',borderRadius:28,background:'rgba(7,17,29,.78)',boxShadow:'0 30px 90px rgba(0,0,0,.4)',backdropFilter:'blur(22px)'}}>
        ...
        <p style={{fontSize:17,lineHeight:1.6,opacity:.68,margin:'0 0 30px'}}>{body}</p>
        <Link href="/" style={{display:'inline-flex',alignItems:'center',justifyContent:'center',padding:'13px 22px',borderRadius:999,background:'#fff',color:'#07111d',fontWeight:650}}>{backLabel}</Link>
      </section>
    </div>
  </main>;
}
