...
export default function SkillsPage(){const[score,setScore]=useState(0);useEffect(()=>{let raf=0;const start=performance.now();const tick=(now:number)=>{const p=Math.min(1,(now-start)/1300);setScore(Math.round(82*(1-Math.pow(1-p,3))));if(p<1)raf=requestAnimationFrame(tick)};raf=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf)},[]);return <main className={styles.page} style={{paddingLeft:'94px'}}>
  ...
  <section className={styles.board}>
    ...
    <article className={`${styles.card} ${styles.analysisCard}`}>
      <img src="/skills-analysis-phone.webp" alt="" aria-hidden="true" style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',objectPosition:'center center',zIndex:0,pointerEvents:'none'}}/>
      <div style={{position:'absolute',inset:0,zIndex:1,pointerEvents:'none',background:'linear-gradient(90deg,rgba(5,20,31,.88) 0%,rgba(5,20,31,.54) 28%,rgba(5,20,31,.04) 55%,rgba(5,20,31,.08) 75%,rgba(5,20,31,.42) 100%)'}}/>
      <div className={styles.analysisCopy} style={{position:'relative',zIndex:2}}><div className={styles.cameraIcon}>▣</div><div><h3>Analise suas reuniões</h3><p>Envie sua gravação e receba uma<br/>análise completa com insights<br/>de performance.</p><button><b>↥</b>Selecionar arquivo</button><small>MP4, MOV ou WEBM</small></div></div><div/><div className={styles.capture} style={{position:'relative',zIndex:2}}><p>CAPTURE.<br/>ANALISE.<br/>EVOLUA.</p><i/><span>Do seu smartphone<br/>para insights reais.</span></div>
    </article>
  </section>
</main>}
