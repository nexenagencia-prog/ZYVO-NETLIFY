...
export default function SkillsPage(){const[score,setScore]=useState(0);useEffect(()=>{let raf=0;const start=performance.now();const tick=(now:number)=>{const p=Math.min(1,(now-start)/1300);setScore(Math.round(82*(1-Math.pow(1-p,3))));if(p<1)raf=requestAnimationFrame(tick)};raf=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf)},[]);return <main className={styles.page} style={{paddingLeft:'94px'}}>
  ...
  <section className={styles.board}>
    ...
    <article className={`${styles.card} ${styles.analysisCard}`} style={{backgroundImage:"linear-gradient(90deg,rgba(5,20,31,.94) 0%,rgba(5,20,31,.72) 28%,rgba(5,20,31,.12) 56%,rgba(5,20,31,.12) 76%,rgba(5,20,31,.58) 100%),url('/skills-analysis-phone.webp')",backgroundSize:'cover',backgroundPosition:'58% center',backgroundRepeat:'no-repeat'}}><div className={styles.analysisCopy}><div className={styles.cameraIcon}>▣</div><div><h3>Analise suas reuniões</h3><p>Envie sua gravação e receba uma<br/>análise completa com insights<br/>de performance.</p><button><b>↥</b>Selecionar arquivo</button><small>MP4, MOV ou WEBM</small></div></div><div/><div className={styles.capture}><p>CAPTURE.<br/>ANALISE.<br/>EVOLUA.</p><i/><span>Do seu smartphone<br/>para insights reais.</span></div></article>
  </section>
</main>}
